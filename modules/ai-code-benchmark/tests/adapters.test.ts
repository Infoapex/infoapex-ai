...
test("candidate telemetry accepts the worker's current platform-default state layout", () => {
  ...
  const temporaryBase = mkdtempSync(join(tmpdir(), "ai-code-benchmark-local-state-"));
  const defaultBase = process.platform === "darwin" ? join(homedir(), "Library", "Application Support") : temporaryBase;
  ...
  const environmentName = process.platform === "win32" ? "LOCALAPPDATA" : "XDG_STATE_HOME";
  const prior = process.env[environmentName];
  if (process.platform !== "darwin") process.env[environmentName] = temporaryBase;
  try {
    ...
    rmSync(repository, { recursive: true, force: true }); rmSync(temporaryBase, { recursive: true, force: true }); rmSync(runRoot, { recursive: true, force: true });
  }
});
