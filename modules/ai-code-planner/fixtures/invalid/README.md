...
| `dangling-criterion-id.json` | `GATE_CRITERION_REF` → `UNKNOWN_CRITERION` | TASK-A explicitly lists `criterionIds: ["AC-999"]` on a gate, but `"AC-999"` is not declared by that task. |
