...
export function runClaude(options: ClaudeRunOptions): ClaudeRunReport {
  ...
  for (const taskId of graph.topologicalOrder) {
    ...
    if (budgetEvaluation.status === "BLOCK") {
      ...
      writeJson(runEvidencePath, aggregateEvidenceFor(compile.runId, "claude", doctor.version, usageTotals, gateResults));
      eventLog.append({
...
function continueFromCommittedTask(input: {
  ...
  writeJson(join(input.taskRoot, "evidence.json"), evidenceFor(input.compile.runId!, "claude", input.engineVersion, unknownUsage(), taskGateReport.results));
