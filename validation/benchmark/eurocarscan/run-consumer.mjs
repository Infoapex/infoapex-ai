...
function scaffoldingPlan(workspace, task) {
  ...
  const prompt = String(task.prompt); const body = { workerContractVersion: "1.1", goal: prompt, tasks: [{ id: `CONSUMER-${String(task.id).toUpperCase()}`, kind: "backend", role: prompt, dependsOn: [], requiredInputs: [], allowedPaths: Array.isArray(task.scope?.allow) ? task.scope.allow : [], forbiddenPaths: [".git/**", ".infoapex-ai/**", "mockup/**", "*.env", "*.log"], expectedArtifacts: Array.isArray(task.scope?.allow) ? task.scope.allow : [], acceptanceCriteria: [prompt], verify: ["python -m unittest discover -s tests"], traceability: { acceptanceCriteria: [{ criterionId: "AC-CONSUMER-01", text: prompt }], gates: [{ gateId: "G-CONSUMER-01", command: "python -m unittest discover -s tests", evidenceContract: "Consumer tests pass.", criterionIds: ["AC-CONSUMER-01"] }] }, concurrencyKeys: [String(task.id)], risk: String(task.risk) }] };
...
function assertFrozenRepositoryRevision(repository, loadedSuite) { const expected = [...loadedSuite.tasks].map((task) => task.value.initialState?.revision).find((revision) => typeof revision === "string" && revision !== "0000000"); if (!expected) throw new Error("Consumer suite has no frozen repository revision."); let actual; try { actual = execFileSync("git", ["rev-parse", "HEAD"], { cwd: repository, encoding: "utf8" }).trim(); } catch { throw new Error("Consumer benchmark repository must be a Git worktree at the frozen revision."); } if (!actual.startsWith(expected)) throw new Error(`Frozen consumer suite targets revision ${expected}, but repository is at ${actual}. Create a new suite and experiment revision after architecture changes.`); }
