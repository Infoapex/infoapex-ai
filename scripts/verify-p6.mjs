...
check("governance-files", ["SECURITY.md", "CONTRIBUTING.md", "CHANGELOG.md", ".github/CODEOWNERS", "docs/P6-DATA-CLASSIFICATION.md", "docs/P6-THREAT-MODEL.md", "docs/P6-OPERATIONS.md", "docs/P6-SUPPORT-POLICY.md", "docs/P6-PILOT-VALIDATOR.md", "docs/P6-SOLO-PROFILE.md"].every((path) => existsSync(join(root, path))));
...
try { const policy = JSON.parse(readFileSync(join(root, "validation/p6/solo-release-policy.json"))); check("solo-release-policy", policy.schemaVersion === "1.0" && policy.profile === "solo-maintainer" && policy.publicationBoundary.automaticPublication === false, "solo profile is explicit and publication remains manual"); } catch (error) { check("solo-release-policy", false, String(error)); }
...
try { execFileSync(process.execPath, [join(root, "scripts/check-npm-package.mjs")], { cwd: root, stdio: "pipe" }); check("npm-package-content", true); } catch (error) { check("npm-package-content", false, String(error)); }
